...
const CurrentEcho = computed<Echo | undefined>(() => CurrentEchoes.value.find(x => x.EquipedSlot === props.echoSlot))

onMounted(() => {
  if (CurrentEcho.value !== undefined && CurrentEcho.value.Id !== -1) {
      ...
      <div class="flex items-center justify-between px-4 py-3">
        <div class="flex items-center gap-6">
          <h2 class="text-lg font-semibold text-white flex items-center gap-2">
            <UIcon name="i-lucide-edit-3" class="h-5 w-5 text-primary-400" />
            Edit Echo
          </h2>
          <!-- Progress Steps -->
